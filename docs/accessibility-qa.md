...
This checklist complements the automated `npm run a11y` axe smoke suite. Run it before shipping changes to public navigation, the five-course reader, the IDE, course-resource rendering, or Julio's private teacher account.

## Screen Reader Pass

- On macOS with VoiceOver, open `/`, `/ide`, `/course-resource?asset=%2Fcourse-assets%2Fpython%2Fturtle-project-reference.md%23turtle-command-reference&label=Turtle+command+reference`, and `/admin`. Use `VO + Right Arrow`, then navigate by headings, landmarks, buttons, form controls, and links.
