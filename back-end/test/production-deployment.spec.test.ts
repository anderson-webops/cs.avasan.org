...
describe("versioned full-stack production deployment", () => {
	...
	it("publishes one non-cacheable release identity for the site and API", () => {
		...
		expect(rootPackage.version).toBe("2.7.118");
		expect(compose.match(/CS_RELEASE_VERSION: \$\{CS_RELEASE_VERSION:-2[.]7[.]118\}/g)).toHaveLength(2);
		...
		expect(api).not.toContain("\n        environment:\n            SOURCE_REVISION:");
		expect(frontendDockerfile).toContain("ARG CS_RELEASE_VERSION=2.7.118");
		expect(frontendDockerfile).toContain("ARG SOURCE_REVISION=unknown");
		expect(apiDockerfile).toContain("ARG CS_RELEASE_VERSION=2.7.118");
